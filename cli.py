...
def main():
    ...
    subtitles , ass_path = get_subtitles(
...
def get_subtitles(audio_paths: list, output_srt: bool, output_dir: str, transcribe: callable):
    ...
    for path, audio_path in audio_paths.items():
        ...
        print(
            f"Generating srt subtitles for {filename(path)}... This might take a while."
        )
        ...
        ass_path = output_dir if output_srt else "temp/"
        ass_path = os.path.join(ass_path, f"{filename(path)}.ass")

        print(
            f"Generating ass subtitles for {filename(path)}... This might take a while."
        )
        
        with open(ass_path,"w", encoding="utf-8") as ass:
            write_ass(result["segments"], file=ass)

        subtitles_path[path] = srt_path
        print(ass_path)

    return subtitles_path, ass_path
